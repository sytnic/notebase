## Как отвязать (удалить) удалённый репозиторий от локального проекта
...
Чтобы клонировать локальный репозиторий с помощью Git, нужно: 

- Перейти в целевой каталог — изменить рабочий каталог на место, где нужно создать копию.  
- Выполнить команду git clone с указанием пути к локальному репозиторию. Синтаксис: 

      git clone /path/to/your/repository.
 
Некоторые советы:  
- Можно указать другое имя для клонированного каталога — добавить второй аргумент в команду git clone. Например: 

      git clone /path/to/your/repository new-directory-name.

- Для больших репозиториев можно использовать опцию --depth — создать поверхностный клон с ограниченной историей.
