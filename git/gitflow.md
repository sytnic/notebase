...
По теме ещё: https://www.dev-notes.ru/articles/git/choosing-branching-strategy/  
...
Также чтобы сохранить изменения, внесенные в ветку релиза, нам нужно объединить их с веткой develop  
...
Тут также надо не забыть запушить релиз в онлайн (это будет отдельная папка-версия в онлайне), а потом удалить локальную ветку релиза, если она уже запушена. - dm.s
...
Если в данный момент существует ветвь релиза (release branch), то ветвь исправления (hotfix branch) должна вливаться в неё, а не в ветвь разработки (develop). Тогда изменения вернутся в develop после слияния release-ветки в develop и master.
